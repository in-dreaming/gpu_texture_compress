...
> **重大更新 (2026-06-02 实验三)**：引入可变 weight grid + 双/三 mode block-mode search，4 个格式跨过 40 dB（4×4 / 5×4 / 5×5 / 6×5），其他格式提升 +1~+2 dB。

| 格式 | 当前 PSNR (dB) | Δ vs 原始 baseline | SSIM | FLIP | Time (ms) | 编码模式 |
|------|-----------|--------------------|------|------|-----------|----------|
| ASTC 4×4 | **47.79** | +40.23 | 0.9912 | 0.0027 | 2.5 | 4×4 grid + Q12 + Q256 ✅ |
| ASTC 5×4 | **45.30** | +37.74 | 0.9890 | 0.0036 | 1.6 | 1:1 grid (5×4) + Q8 ✅ 🚀 |
| ASTC 5×5 | **41.89** | +34.33 | 0.9745 | 0.0064 | 1.9 | dual: 5×5+Q5 ∨ 4×4+Q12 ✅ |
| ASTC 6×5 | **40.18** | +32.62 | 0.9803 | 0.0068 | 2.2 | triple: 5×5+Q5 ∨ 4×4+Q12 ∨ 6×5+Q4(1:1) ✅ |
| ASTC 6×6 | **36.22** | +28.66 | 0.9616 | 0.0100 | 2.6 | dual: 5×5+Q5 ∨ 4×4+Q12 |
| ASTC 8×5 | **35.47** | +27.91 | 0.9543 | 0.0102 | 2.7 | dual |
| ASTC 8×6 | **34.53** | +25.97 | 0.9408 | 0.0117 | 2.4 | dual |
| ASTC 8×8 | **33.48** | +25.92 | 0.9301 | 0.0136 | 2.5 | dual |
| ASTC 10×5 | **34.05** | +26.49 | 0.9388 | 0.0117 | 2.6 | dual |
| ASTC 10×6 | **33.42** | +25.86 | 0.9286 | 0.0131 | 2.4 | dual |
| ASTC 10×8 | **32.62** | +25.06 | 0.9128 | 0.0150 | 2.4 | dual |
| ASTC 10×10 | **31.96** | +24.40 | 0.9050 | 0.0158 | 2.5 | dual |
| ASTC 12×10 | **31.37** | +23.81 | 0.8987 | 0.0167 | 2.5 | dual |
| ASTC 12×12 | **30.87** | +23.31 | 0.8916 | 0.0177 | 2.7 | dual |
...
9. **可变 weight grid（5×5 in 5×5..12×12）** — 替换 4×4 grid 子采样为 5×5 全像素 PCA + bilinear；ASTC_5×5 +6.6 dB，全 12 格式累计 +22.5 dB
10. **双 mode block-mode search（5×5+Q5 ∨ 4×4+Q12）** — 每块按重建误差选最低，自适应高频/低频；全 12 格式额外 +0.3~0.9 dB
11. **1:1 grid 编码器（grid = block size）** — 5×4 块用 5×4 grid + Q8（45.30 dB）；编码器看到全部像素，无解码器双线性平滑损失
12. **三 mode block-mode search（5×5+Q5 ∨ 4×4+Q12 ∨ 1:1+QN）** — 6×5 块用 6×5+Q4 1:1 路径（40.18 dB），按块挑选最优
