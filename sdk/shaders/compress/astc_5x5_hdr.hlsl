#ifndef ASTC_5x5_HDR_HLSL
#define ASTC_5x5_HDR_HLSL

//=============================================================================
// ASTC 5x5 HDR Compression
// 25 pixels -> 4x4 weight grid
//=============================================================================
...
uint4 compress_astc_5x5_hdr(float4 pixels[25])
{
    ...
    [unroll] for (int gy = 0; gy < 4; gy++) {
        [unroll] for (int gx = 0; gx < 4; gx++) {
            uint px = ((uint)gx * 4 + 1) / 3;
            uint py = ((uint)gy * 4 + 1) / 3;
            float3 hdr_rgb = pixels[py * 5 + px].rgb;
            texels[gy * 4 + gx] = float4(hdr_rgb / max_hdr_val * 255.0, pixels[py * 5 + px].a * 255.0);
        }
    }
