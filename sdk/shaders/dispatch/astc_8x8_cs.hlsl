// ASTC 8x8 Compute Shader Dispatch
#include "common/gtc_interface.hlsl"
#include "compress/astc_8x8.hlsl"
...
void MainCS(uint3 DTid : SV_DispatchThreadID) {
    ...
    [unroll] for (int py = 0; py < 8; py++)
        [unroll] for (int px = 0; px < 8; px++) {
            int2 coord = int2((int)DTid.x * 8 + px, (int)DTid.y * 8 + py);
    ...
    OutputBlocks[blockIndex] = compress_astc_8x8(pixels);
}
