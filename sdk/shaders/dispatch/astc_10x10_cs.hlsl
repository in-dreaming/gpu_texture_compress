// ASTC 10x10 Compute Shader Dispatch
#include "common/gtc_interface.hlsl"
#include "compress/astc_10x10.hlsl"
...
void MainCS(uint3 DTid : SV_DispatchThreadID) {
    ...
    [unroll] for (int py = 0; py < 10; py++)
        [unroll] for (int px = 0; px < 10; px++) {
            int2 coord = int2((int)DTid.x * 10 + px, (int)DTid.y * 10 + py);
    ...
    OutputBlocks[blockIndex] = compress_astc_10x10(pixels);
}
