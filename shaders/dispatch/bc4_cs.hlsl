// dispatch/bc4_cs.hlsl - Compute shader entry point for BC4 compression
// Dispatches one thread per 4x4 block, loads .r channel from SourceTexture,
// calls pure compress_bc4() function.
...
void MainCS(uint3 DTid : SV_DispatchThreadID) {
    ...
    [unroll] for (int py = 0; py < 4; py++)
        [unroll] for (int px = 0; px < 4; px++) {
            ...
            values[py * 4 + px] = SourceTexture.Load(int3(coord, 0)).r;
        }

    uint blockIndex = DTid.y * BlocksX + DTid.x;
    OutputBlocks[blockIndex] = compress_bc4(values);
}
