...
// Block: 6x5 = 30 pixels
// Grid:  4x4 = 16 weights (proportional mapping with interpolation)
...
uint4 compress_astc_6x5(float4 pixels[30])
{
    ...
    // 4. Compute weights: map 4x4 grid to 6x5 pixel block
    //    px = (gx * 5 + 1) / 3, py = (gy * 4 + 1) / 3
    ...
    // 5. Pack and return with correct block mode for 6x5
    return astc_pack_block_with_mode(ASTC_BLOCK_MODE_6x5_Q4, endpoints, weights);
}
