...
uint4 compress_astc_12x10(float4 pixels[120])
{
    ...
    // 4. Compute weights: map 4x4 grid to 12x10 pixel block
    //    px = (gx * 11 + 1) / 3, py = (gy * 9 + 1) / 3
    ...
    // 5. Pack and return with correct block mode for 12x10
    return astc_pack_block_with_mode(ASTC_BLOCK_MODE_12x10_Q4, endpoints, weights);
}
