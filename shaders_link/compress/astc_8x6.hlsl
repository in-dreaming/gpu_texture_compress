...
uint4 compress_astc_8x6(float4 pixels[48])
{
    ...
    // 4. Compute weights: map 4x4 grid to 8x6 pixel block
    //    px = (gx * 7 + 1) / 3, py = (gy * 5 + 1) / 3
    ...
    // 5. Pack and return with correct block mode for 8x6
    return astc_pack_block_with_mode(ASTC_BLOCK_MODE_8x6_Q4, endpoints, weights);
}
