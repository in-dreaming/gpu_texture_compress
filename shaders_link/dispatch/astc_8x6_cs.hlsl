// ASTC 8x6 Compute Shader Dispatch
#include "common/gtc_interface.hlsl"
#include "compress/astc_8x6.hlsl"
...
void MainCS(uint3 DTid : SV_DispatchThreadID) {
    ...
    [unroll] for (int py = 0; py < 6; py++)
        [unroll] for (int px = 0; px < 8; px++) {
            int2 coord = int2((int)DTid.x * 8 + px, (int)DTid.y * 6 + py);
    ...
    OutputBlocks[blockIndex] = compress_astc_8x6(pixels);
}
