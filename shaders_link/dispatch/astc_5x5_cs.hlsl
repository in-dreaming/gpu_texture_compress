// ASTC 5x5 Compute Shader Dispatch
#include "common/gtc_interface.hlsl"
#include "compress/astc_5x5.hlsl"
...
void MainCS(uint3 DTid : SV_DispatchThreadID) {
    ...
    [unroll] for (int py = 0; py < 5; py++)
        [unroll] for (int px = 0; px < 5; px++) {
            int2 coord = int2((int)DTid.x * 5 + px, (int)DTid.y * 5 + py);
    ...
    OutputBlocks[blockIndex] = compress_astc_5x5(pixels);
}
