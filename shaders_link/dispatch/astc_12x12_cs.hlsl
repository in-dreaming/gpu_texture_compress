// ASTC 12x12 Compute Shader Dispatch
#include "common/gtc_interface.hlsl"
#include "compress/astc_12x12.hlsl"
...
void MainCS(uint3 DTid : SV_DispatchThreadID) {
    ...
    float4 pixels[144];
    [unroll] for (int py = 0; py < 12; py++)
        [unroll] for (int px = 0; px < 12; px++) {
            int2 coord = int2((int)DTid.x * 12 + px, (int)DTid.y * 12 + py);
    ...
    OutputBlocks[blockIndex] = compress_astc_12x12(pixels);
}
